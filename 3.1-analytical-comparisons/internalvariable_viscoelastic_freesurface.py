...
# There are four default tests:
# 1) elastic-compressible case limit (dt << maxwell time, 1 step)
# 2) elastic-incompressible case limit (dt << maxwell time, 1 step)
# 3) viscoelastic-compressible (dt ~ maxwell time)
# 4) viscoelastic-incompressible (dt ~ maxwell time)
...
def run_benchmark(case_name):
    ...
    refinements = params[case_name]["refinements"]
    params[case_name].pop("refinements")  # Don't pass this to viscoelastic_model
    dt_factors = dtf_start / (2 ** np.arange(refinements))
